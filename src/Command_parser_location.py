...
def in_region(point, point_approx, degree = 0.2):
    for x in range(len(point)):
        procent_difference = abs(point[x] - point_approx[x]) / (abs(point[x] + point_approx[x]) / 2) 
...
class CommandParserLocation:
    ...
    def __eq__(self, __o: object) -> bool:
        return None not in [self.graph, self.source_node, self.target_node, self.origin, self.target]
    ...
    def _region_select(self):
        ...
        print(f"""Please select your region, i.e. country or city, by entering a search query""")
        ...
        while True:
            ...
            try:
                ...
                raise CommandExceptionLocation(f"{command} is not a valid dataset. Use command \'places\' to see the available datasets.")
    ...
    def _place_select(self, type):
        if not self.region:
            raise CommandExceptionLocation("No region selected: please select region first")
        place_address = self.origin if type == 'origin' else self.target # the current value or none
        place_node = self.source_node if type == 'origin' else self.target_node # the current value or none
        region = self.region
        print(f"""Please select your {type} by entering a search query.
            Current region: {region}""")
        self._get_help("place")
        while True:
            command = input("Origin Select> ").lower() if type == 'origin' else input("Target Select> ").lower()
            if command == "back":
                break
            if command == "help":
                self._get_help("place")
            ...
            try:
                ...
                if not in_region(place_coords, node_coords):
                    raise CommandExceptionLocation(f"Point \'{place_address}\' not in \'{region}\'")
                break
    ...
    def set_target_values(self, place_address, place_node):
        ...
        if self.origin == self.target or self.source_node == self.target_node:
            self.target = None
            self.target_node = None
            raise CommandExceptionLocation("Origin and target cannot be the same; please select target again.")
    ...
    def set_origin_values(self, place_address, place_node):
        ...
        if self.origin == self.target or self.source_node == self.target_node:
            self.origin = None
            self.source_node = None
            raise CommandExceptionLocation("Origin and target cannot be the same; please select origin again.")
